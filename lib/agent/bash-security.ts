import { extractHeredocs } from "@/lib/agent/bash-heredoc"
import {
  hasMalformedTokens,
  hasShellQuoteSingleQuoteBug,
  tryParseShellCommand,
} from "@/lib/agent/bash-shell-quote"
...
type TreeSitterAnalysis = {
  dangerousPatterns: { hasHeredoc: boolean }
  hasActualOperatorNodes: boolean
  quoteContext: QuoteExtraction
}
...
export function bashCommandNeedsApproval(command: string) {
  return bashCommandIsSafe_DEPRECATED(command).behavior === 'ask'
}

function collectCommandCandidates(value: unknown, output: Set<string>) {
  if (!value || typeof value !== 'object') return

  if (Array.isArray(value)) {
    for (const item of value) {
      collectCommandCandidates(item, output)
    }
  ...
  for (const [key, nestedValue] of Object.entries(value)) {
    ...
    if (
      ...
      output.add(nestedValue)
      continue
    }

    collectCommandCandidates(nestedValue, output)
  }
}

export function bashPermissionInputNeedsApproval(inputPreview: string) {
  const candidates = new Set<string>()

  try {
    collectCommandCandidates(JSON.parse(inputPreview), candidates)
  } catch {
    candidates.add(inputPreview)
  }
  ...
  return [...candidates].some(command => bashCommandNeedsApproval(command))
}
