...
/** Build a user-visible error when a provider/API request is rejected. */
export function formatProviderRequestError({
  ...
  const message = messageCandidates[0] || fallback
  ...
  const parts = [message]
  if (codeCandidates[0]) {
    parts.push(`code ${codeCandidates[0]}`)
  }
  if (typeof status === "number" && status > 0) {
    parts.push(`HTTP ${status}`)
  }

  const haystack = `${message} ${codeCandidates.join(" ")}`.toLowerCase()
  const blocked =
    status === 401 ||
    status === 403 ||
    status === 451 ||
    /client|forbidden|not allow|denied|unauthorized|无权限|拦截|拒绝|不允许|green|not enabled|not permitted/.test(
      haystack
    )
