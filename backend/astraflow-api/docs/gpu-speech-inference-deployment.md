...
每张 RTX 4090 有 24 GB GDDR6X 显存。ASR 与标题模型分别独占一张卡，两个 vLLM 实例的显存利用率均从 `0.85` 起步。这样避免两个实例同时 profiling 时互相改变空闲显存，也为各自的 CUDA graph、激活值和 KV cache 留出约 15% 余量。
...
astraflow-api :8000/:9000
        ...
        ▼
inference-gateway（独立 CPU Deployment）
        ├── HTTP astraflow-asr:8001 ── Qwen3-ASR-1.7B ── 4090 节点 A
        └── HTTP astraflow-title:8002 ─ Qwen3-8B-AWQ ─── 4090 节点 B
```
...
3. 确认 UK8S 至少有两个 4090 Pro 节点带有目标标签：
...
Helm 创建两个各自申请 `nvidia.com/gpu: 1` 的模型 Deployment，并配置基于 `kubernetes.io/hostname` 的强制 Pod 反亲和，因此二者一定落到不同节点：

- `astraflow-asr`：只加载 `Qwen3-ASR-1.7B`，监听 `8001`。
- `astraflow-title`：只加载 `Qwen3-8B-AWQ` INT4，监听 `8002`。
- `astraflow-inference-gateway`：独立 CPU Deployment，通过两个 ClusterIP 调用模型。
- 两个模型 Pod 只读挂载同一个 US3 PVC，各自只复制需要的权重到节点本地 `emptyDir`。
- 两个模型容器分别固定请求并限制为 `8 CPU / 16 GiB`，适配每台 `16 CPU / 32 GiB` 的独占节点，同时给系统组件和 US3FS 留出一半主机资源。
...
# 独立 gateway Deployment
- name: ASR_BASE_URL
  value: http://astraflow-asr:8001
- name: TITLE_BASE_URL
  value: http://astraflow-title:8002
...
三个 Service 都只在集群内可见。不要把模型端口 8001、8002 或内部 gRPC 端口 9100 暴露到公网。
