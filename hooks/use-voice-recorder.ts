...
export function useVoiceRecorder() {
  const runtimeRef = React.useRef<RecorderRuntime | null>(null)
  const timerRef = React.useRef<number | null>(null)
  const waveformLevelsRef = React.useRef<number[]>([])
  const waveformLastEmitAtRef = React.useRef(0)
  const [isRecording, setIsRecording] = React.useState(false)
  const [durationMs, setDurationMs] = React.useState(0)
  const [waveformLevels, setWaveformLevels] = React.useState<number[]>([])
  ...
  const teardownRuntime = React.useCallback(async () => {
    ...
    try {
      ...
      processorNode.onaudioprocess = (event) => {
        ...
        for (let sampleIndex = 0; sampleIndex < frameCount; sampleIndex += 1) {
          monoSamples[sampleIndex] =
            (monoSamples[sampleIndex] ?? 0) / normalizer
        }
  ...
  const stopRecording = React.useCallback(async () => {
    ...
    return {
      ...
      durationMs: Math.max(
        1,
        Math.round((resampledSamples.length / TARGET_SAMPLE_RATE) * 1_000) ||
          recorded.durationMs
      ),
    } satisfies VoiceRecordingPayload
  }, [teardownRuntime])
