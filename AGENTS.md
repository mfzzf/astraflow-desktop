...
- Single-app layout: the Next.js app lives at the repository root (`app`, `components`, `hooks`, `lib`); backend API code lives under `backend/astraflow-api`.
- Shared shadcn/ui components live under `components/ui`.
- Use `bunx --bun shadcn@latest add <component>` from the repository root for shadcn components.
...
- For lightweight details, show as much useful information inline as fits, then use a small Popover or simple secondary floating panel from an `i`/info control for overflow details. Avoid right-side Sheets for small read-only details; reserve Sheets for larger forms and workflows.
...
- In the persistent App Router shell, keep previous-view transition state local to the transition component instance. Do not persist it in `sessionStorage`, because refreshes, deep links, and login redirects can replay stale page directions.
