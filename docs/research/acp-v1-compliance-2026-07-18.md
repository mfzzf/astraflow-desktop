...
| Initialization | Sends ACP v1, implementation metadata, and truthful local/remote client capabilities; rejects a negotiated version the client cannot support. | Always negotiates its supported ACP v1 version and advertises only implemented capabilities and content types. | Initialize/version/capability conformance tests |
...
| Session setup | Capability-gates `session/resume`, `session/load`, MCP transports, and additional directories. Load notifications are observed before the load response and state-bearing replay is reconciled. | Implements new/load/resume; load replays user, assistant, thought, tool-call, and tool-result history before returning, while resume does not replay. | Runtime replay test |
| Session list | Provides a capability-gated, cwd-filtered paginated session browser. | Implements cwd-bound opaque cursors, deterministic 50-item pages, titles, timestamps, cwd, and additional directories. | Runtime pagination test |
| Session delete / close | Provides capability-gated close/delete actions and confirmation for destructive deletion; transport cleanup calls `session/close` when available. | Implements idempotent close/delete, cancels active work, waits for prompt cleanup, and removes deleted checkpoints from list results. | Runtime lifecycle test |
| Prompt turn | Sends protocol-native prompt blocks and consumes ordered session updates until the prompt stop response. Standard message IDs remain opaque and distinct. | Merges the inbound JSON-RPC request signal with `session/cancel`, streams updates, and returns the ACP stop reason and usage. | Client conformance and runtime prompt tests |
...
| Tool calls | Preserves replacement vs patch semantics, title, kind, status, locations, structured content, raw input/output, and nested `_meta`; renders diff, terminal, resource, and content results. Streaming output is redacted before it enters retained state. | Continues publishing ACP tool calls/updates and permission requests with stable IDs. | Structured-event tests and runtime tool tests |
| Agent plan | Supports stable replacement plans and experimental identified items/markdown/file plan updates and removal without conflating concurrent plans. | Publishes stable ACP plans through the existing Pi plan forwarder. | Plan identity/removal conformance test |
| Session config options | Renders ordered select/group/boolean controls, sends the boolean discriminator, replaces state with the complete response, and tracks agent notifications. Config options supersede legacy modes in the UI. | Implements mode, model, and thought-level config options, returns complete state, and persists session selections. | Runtime config test |
...
| Slash commands | Preserves agent-provided command descriptors and live command updates in runtime state and Studio command discovery. Agent-advertised commands take precedence over colliding built-ins, skills, and MCP commands and are delivered to the runtime without client-side reinterpretation. | Existing runtime command publication remains protocol-native. | Focused Studio command-precedence and skill-invocation tests |
...
| Terminals | Capability-gates all terminal methods, validates session ownership/cwd, supports create/output/wait/kill/release, removes aborted waiters, flushes trailing decoder state, and enforces exact UTF-8 byte limits across split multibyte chunks. | Uses the client terminal surface through the permission backend. | UTF-8 limit, aborted-waiter, and runtime backend tests |
| Extensibility | Preserves `_meta` on requests, responses, notifications, plan entries, commands, command inputs, content annotations, and implementation data; keeps unknown provider protocol strings open; bounds/redacts opaque values. | Keeps AstraFlow metadata under `_meta.astraflow` without altering base ACP semantics. | Structured content/tool and conformance tests |
...
- Studio controls and rendering: `components/studio-chat/acp-controls.tsx`, `components/studio-message-parts/structured-content.tsx`
...
```bash
bun test tests/acp-v1-conformance.test.ts tests/agent-structured-events.test.ts tests/agent-usage.test.ts
bun test tests/studio-slash-commands.test.ts tests/studio-tool-labels.test.ts
