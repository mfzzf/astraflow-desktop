...
3. **任务 / 会话**：可以归属于某个工作区；也可以是不绑定文件系统的普通对话。
...
### 3.2 StudioSession 显式引用 Workspace
...
绑定工作区的任务不再自行决定本地或远程。任务的文件系统、终端和运行位置全部继承所属工作区。

`workspace_id` 允许为 `NULL`，表示普通对话。普通对话：

- 允许直接创建和发送消息；
- 使用本地会话私有运行目录，不绑定用户项目目录；
- 不显示工作区文件树、工作区终端、Git Review 或 Workspace Artifact；
- 不得因为运行 Agent 或打开 UI 而隐式创建本地 / Sandbox 工作区；
- 用户可在发送第一条消息前选择工作区，选择后再按显式类型锁定传输层。
...
未绑定工作区的普通对话固定使用本地通用聊天模式，但不等价于 Local Workspace：它没有项目根目录，也不开放工作区文件、终端或 Git 能力。
...
- 账号尚无工作区时仍可进入并发送普通对话，不强制弹出创建窗口；用户也可从输入框下方选择器新建并绑定首个工作区；
...
- [x] 没有任何工作区时仍能创建和发送普通对话，并可从统一选择器创建首个工作区。
