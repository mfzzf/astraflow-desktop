...
function discoverSkills() {
  ...
  for (const entry of readdirSync(bundleRoot, { withFileTypes: true })) {
    ...
    const metadataOverride = skillMetadataOverrides[slug]

    skills.push({
      slug,
      version: metadataOverride?.version || frontmatter.version || "1.0.0",
      source:
        metadataOverride?.source || "User-provided AstraFlow built-in skill",
      license:
        metadataOverride?.license ||
        frontmatter.license ||
        "User-provided proprietary",
    })
  }
